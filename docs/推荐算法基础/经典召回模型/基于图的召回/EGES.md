...
![image-20220328133138263](http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328133138263.png)
...
<img src="http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328144516898.png" alt="image-20220328144516898" style="zoom: 67%;" />

其中$M_{ij}$为边$e_{ij}$上的权重，$N_{+}(v_i)$表示节点$v_i$所有邻居节点集合，并且随机游走的转移概率的对每个节点所有邻接边权重的归一化结果。在随即游走之后，每个item得到一个序列，如下图所示：

<img src="https://cdn.jsdelivr.net/gh/swallown1/blogimages@main/images/image-20220418142135912.png" alt="image-20220418142135912" style="zoom:50%;" />

然后类似于word2vec，为每个item学习embedding，于是优化目标如下：

<img src="http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328144931957.png" alt="image-20220328144931957" style="zoom:67%;" />

其中，w 为窗口大小。考虑独立性假设的话，上面的式子可以进一步化简：

<img src="http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328145101109.png" alt="image-20220328145101109" style="zoom:67%;" />

这样看起来就很直观了，在已知物品 i 时，最大化序列中(上下文)其他物品 j 的条件概率。为了近似计算，采样了Negative sampling，上面的优化目标可以化简得到如下式子：

<img src="http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328145318718.png" alt="image-20220328145318718" style="zoom:67%;" />



其中$N(v_i)'$表示负样本集合，负采样个数越多，结果越好。

### 3.3 基于side information的图嵌入（GES）
...
为了解决冷气问题，本文通过使用side information（ 类别，店铺, 价格等）加入模型的训练过程中，使得模型最终的泛化能力体现在商品的side information上。这样通过**side information学习到的embedding来表示具体的商品**，使得相似side information的物品可以得到在空间上相近的表示，进而来增强 BGE。
...
![image-20220328154510666](http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328154510666.png)
...
![image-20220328154950289](http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328154950289.png)

其中 $a_i$ 表示每个side information 用于计算权重的参数向量，最终通过下面的公式得到商品的embedding：

![image-20220328155148241](http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328155148241.png)

这里对参数 $a_v^j$ 先做指数变换，目的是为了保证每个边界信息的贡献都能大于0，然后通过归一化为每个特征得到一个o-1之内的权重。最终物品的embedding通过权重进行加权聚合得到，进而优化损失函数：

![image-20220328155805442](http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328155805442.png)

 y是标签符号，等于1时表示正样本，等于0时表示负样本。$H_v$表示商品 v 的最终的隐层表示，$Z_u$表示训练数据中的上下文节点的embedding。

以上就是这三个模型主要的区别，下面是EGES的伪代码。

<img src="http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328155406291.png" alt="image-20220328155406291" style="zoom:67%;" />

其中**WeightedSkipGram**函数为带权重的SkipGram算法。

![image-20220328155533704](http://ryluo.oss-cn-chengdu.aliyuncs.com/图片image-20220328155533704.png)
